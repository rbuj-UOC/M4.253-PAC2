...
export function searchNumber(matrix, targetNumber) {
  ...
  for (const elem of matrix)
    if (Array.isArray(elem) && searchNumber(elem, targetNumber)) return true;
    else if (isNaN(elem)) continue;
    else if (elem === targetNumber) return true;
  return false;
}
...
export function dashboardCompany(company) {
  if (company == null || company === undefined || !(company instanceof Object)) throw new Error('The value of company is invalid.');
  const result = {};
  ...
  for (const department of company['departments']) {
    ...
    for (const employee of department['employees']) {
      ...
      for (const project of employee['projects']) for (const task of project['tasks']) rank['totalHoursWorked'] += task['hoursWorked'];
...
export class WordsAnalyzer {
  ...
  // Constructor
  constructor(data) {
    if (data == null || data === undefined || !(data instanceof Object)) throw new Error('The list of words is invalid.');
  ...
  set words(data) {
    ...
    if (data === undefined || data == null || !Array.isArray(data)) throw new Error(msg);
    for (let i = 0; i < data.length; i++) if (typeof data[i] !== 'string') throw new Error(msg);
    this._words = [...data];
    this.#countAll();
  }
  // Public methods
  ...
  textExists(textToSearch) {
    for (const word of this._words) if (word.includes(textToSearch)) return true;
    return false;
  }
  ...
  deleteWord(number) {
    ...
    if (number === undefined || isNaN(number) || number <= 0 || number > this._words.length) throw new Error(msg);
    if (number === 1) this._words.shift();
    else if (number === this._words.length) this._words.pop();
    else this._words.splice(number - 1, 1);
    this.#countAll();
  }
  // private
  #countAll() {
